...
func _ready():
	...
	graph_manager.add_component(GraphManager.NodeType.GENERATOR, 100, 100).add_load(EnergyCharge.ChargeType.RED, 1)
	...
	graph_manager.add_component(GraphManager.NodeType.GENERATOR, 500, 100).add_load(EnergyCharge.ChargeType.GREEN, 2)
	...
	# New start index = 6
	graph_manager.add_component(GraphManager.NodeType.GENERATOR, 100, 500).add_load(EnergyCharge.ChargeType.GREEN, 1)
	graph_manager.add_component(GraphManager.NodeType.JUNCTION, 200, 500).input_edge_list = [6, 8]
	...
	graph_manager.add_component(GraphManager.NodeType.GENERATOR, 600, 500).add_load(EnergyCharge.ChargeType.BLUE, 3)
